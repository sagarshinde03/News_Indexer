...
public class DateFilter extends TokenFilter {
	...
	private static Pattern patternHyphen = Pattern.compile("[\\d][\\d][\\d][\\d]\\-[\\d][\\d].*");
	private static Pattern patternBC = Pattern.compile("[\\d]+BC.*");
	private static Pattern patternAD = Pattern.compile("[\\d]+AD.*");
	private static Pattern patternTime = Pattern.compile("[\\d]+:[\\d][\\d][apAP][mM].*");
	private static Pattern patternTimeDD = Pattern.compile("[\\d][\\d]:[\\d][\\d]");
	...
	public void applyFilter() throws TokenizerException {
		...
		try {
			...
			if(patternHyphen.matcher(currentToken).matches()){
			...
			}else if(patternBC.matcher(currentToken).matches()){
				...
				if(patternNumber.matcher(previousToken).matches()){
					StringBuffer strBuffer = new StringBuffer();
					strBuffer.append("-");
			...
			}else if(patternAD.matcher(currentToken).matches()){
				if( (trailingChar = hasTrailingChar(currentToken)) != null ){
					currentToken = currentToken.replace(trailingChar, "");
				}
				StringBuffer strBuffer = new StringBuffer();
				...
				if(patternNumber.matcher(getCurrentStream().previous().getTermText()).matches()){
					StringBuffer strBuffer = new StringBuffer();
					for(int i=0;i<(4-getCurrentStream().getCurrent().getTermText().length());i++){
						strBuffer.append("0");
					}
			...
			}else if(patternTime.matcher(currentToken).matches()){
				if( (trailingChar = hasTrailingChar(currentToken)) != null ){
			...
			} else if(patternTimeDD.matcher(currentToken).matches()){
				...
				if(patternNumber.matcher(currentToken).matches() && Integer.valueOf(currentToken)>=1 && Integer.valueOf(currentToken)<=31){
					...
					if(months.contains(getCurrentStream().next().getTermText())){
						...
						if ( patternNumber.matcher(next).matches() && Integer.valueOf(next) >= 1900 && Integer.valueOf(next) <= 2100 ) {  //may need to change the range		
							year= next;
							getCurrentStream().remove();
						} else {
							year= "1900";
					...
					if(patternNumber.matcher(nextToken1).matches() && Integer.valueOf(nextToken1)>=1900 && Integer.valueOf(nextToken1)<=2100){  //may need to change the range
						year= nextToken1;
						getCurrentStream().remove(); getCurrentStream().previous(); noRemove = true;
					}else if(patternNumber.matcher(nextToken).matches() && Integer.valueOf(nextToken)>=1900 && Integer.valueOf(nextToken)<=2100){  //may need to change the range
						year= nextToken;
					...
					if(patternNumber.matcher(nextToken).matches() && Integer.valueOf(nextToken)>=1 && Integer.valueOf(nextToken)<=31){
						date= nextToken;
						if(noRemove){
							getCurrentStream().remove();
						}
					...
					getCurrentStream().previous();		
				} else if(patternNumber.matcher(currentToken).matches() && Integer.valueOf(currentToken)>=1900 && Integer.valueOf(currentToken)<=2100){
					year=currentToken;
					if(months.contains(getCurrentStream().next().getTermText())){
						month=getCurrentStream().getCurrent().getTermText();
					} else{

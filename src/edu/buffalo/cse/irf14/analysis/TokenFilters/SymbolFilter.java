...
public class SymbolFilter extends TokenFilter {
	...
	private static Pattern patternCom = Pattern.compile(".*[\\.\\'\\?\\-\\!\"()<>].*");
	private static Pattern patternHyphen1 =Pattern.compile("(?i)([a-zA-Z]+)(-)([a-zA-Z]+)");
	private static Pattern patternHyphen2 = Pattern.compile("(?i)([a-zA-Z]+)(-)([a-zA-Z]+)(-)([a-zA-Z]+)");
	...
	public void applyFilter() throws TokenizerException {
		...
		try{
			if( null!=str && !str.isEmpty() && patternCom.matcher(str).matches() ){
				...
				if( patternHyphen1.matcher(str).matches() || patternHyphen2.matcher(str).matches()){
					str = str.replaceAll("-"," ");
